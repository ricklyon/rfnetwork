...
Z0_o, Z0_e = utils.coupled_sline_impedance(s_w, s_sp, b, er)
###
...
current_face = pv.Rectangle([
    (-0.5, s1_y - s_w/2 - 0.001, sub_h + 0.001),
    (-0.5, s1_y + s_w/2 + 0.001, sub_h + 0.001),
    (-0.5, s1_y + s_w/2 + 0.001, sub_h - 0.001),
])
...
s.init_mesh(d0 = 0.3, n0 = 4, d_pec = 0.3, n_min_pec=4, d_sub=0.3, n_min_sub=5, blend_pec=False)
s.init_coefficients()
...
frequency: np.ndarray = np.arange(0.5e9, 3e9, 10e6)
...
vp_e = get_vp(line_v1, line_v2, conv.m_in(1))
