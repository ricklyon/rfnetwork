...
class Solver_3D():
    ...
    def get_sparameters(self, frequency, source_port=1, downsample=False):
        """
        ...
        # plt.plot(src_voltage.squeeze().T)
        # src_vp = src_voltage.squeeze()[int(src_width/2)]
        ...
        # get the frequency domain voltage across each port termination
        V_term = np.zeros((nports, nfrequency), dtype=np.complex128)
        for i, p in enumerate(self.ports):

            # voltage generated by termination due the current through it. 
            vp = self.vi_probe_values(f"port{i+1}") * p["r0"]

            # h-fields are 1/2 time step ahead of the e-fields. Dely current so they are at the same time step
            V_term[i] = utils.dtft(vp, frequency, 1 / self.dt, downsample) #* np.exp(-1j * frequency * 2 * np.pi * (self.dt / 2))
