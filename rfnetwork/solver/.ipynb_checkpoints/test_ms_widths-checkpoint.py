...
class Solver_SingleLayer():
    ...
    def run(self, port, v_waveform, gif_step=10):
        ...
        self.gif_fields_ex = np.zeros(((Nt // self.gif_step) + 1, Nx, Ny+1, Nz+1), dtype=dtype_)
        ...
        for k in list(self.ports.keys()):
            self.ports[k]["v"] = np.zeros(Nt, dtype=dtype_)
            self.ports[k]["i"] = np.zeros(Nt, dtype=dtype_)
            self.ports[k]["hx"] = np.zeros((Nt, 2), dtype=dtype_)
            self.ports[k]["hy"] = np.zeros((Nt, 2), dtype=dtype_)
            self.ports[k]["hz"] = np.zeros((Nt, 2), dtype=dtype_)
        ...
        for n in range(Nt -  1):
            ...
            for k, p in self.ports.items():
                x, y, z = list(p.values())[:3]

                self.ports[k]["v"][n + 1] = -np.sum(ez[x, y, :self.ms_z] * dz[:self.ms_z])
                 
                # average along z
                ix = (np.mean(hx[x, y-1, :self.ms_z]) - np.mean(hx[x, y, :self.ms_z])) * dx_h[x - 1]
                iy = (-np.mean(hy[x-1, y, :self.ms_z]) + np.mean(hy[x, y, :self.ms_z])) * dy_h[y - 1]

                self.ports[k]["i"][n + 1] = ix + iy

                self.ports[k]["hx"][n+1] = hx[x, y-1:y+1, 1]
                self.ports[k]["hy"][n+1] = hy[x-1:x+1, y, 1]
                self.ports[k]["hz"][n+1] = hz[x-1:x+1, y, 1]

            if (n % self.gif_step) == 0:
                self.gif_fields[n // self.gif_step] = ez
                self.gif_fields_ex[n // self.gif_step] = ex
            ...
            for k, p in self.c_probes.items():
                ...
                xp = x
                c1 = np.sum((hy[xp, y1:y2+2, self.ms_z-1] - hy[xp, y1:y2+2, self.ms_z]) * dy_h[y1-1:y2+1]) + (-hz[xp, y1-1, 1] + hz[xp, y2+1, self.ms_z]) * dz_h[self.ms_z-1]
                xp = x - 1
                c2 = np.sum((hy[xp, y1:y2+2, self.ms_z-1] - hy[xp, y1:y2+2, self.ms_z]) * dy_h[y1-1:y2+1]) + (-hz[xp, y1-1, 1] + hz[xp, y2+1, self.ms_z]) * dz_h[self.ms_z-1]
                self.c_probes[k]["values"][n+1] = (c1 + c2) / 2
    ...
    def add_port(self, name, ez_x, ez_y, r0=None):
        # resistive load
        r_x = ez_x
        r_y = ez_y
        ...
        # voltage sources
        self.ports[name] = dict(x=r_x, y=r_y, z=r_z, Vs_a=1 / (denom * rterm * self.ms_z))
    ...
    def get_ba(self, port, z0=50, vsrc=None):

        v1 = self.ports[port]["v"] 
        i1 = self.ports[port]["i"] 
...
s.add_port("p1", p1_x, p1_y, r0=50)
s.add_port("p2", p2_x, p2_y, r0=50)
...
s.add_c_probe("probe1", p1_x+5, p1_y-1, p1_y)
s.add_v_probe("probe1", p1_x+5, p1_y)

fig, ax = plt.subplots(1, 1, figsize=(10, 5))
plt.stem(s.Cb["ex_y"][15, :, 2])
plt.xticks(np.arange(Ny));
...
# a = conv.m_in(0.005)
# s.Db["hz_y"][p1_x:, p1_y-2, 1] = (2 * s.dt) / (u0 * np.log(s.dy[p1_y - 1] / a))
...
rs = (np.max(np.abs(vsrc)) / np.max(np.abs(v1)))
rs1 = (np.max(np.abs(v1)) / np.max(np.abs(vsrc)))
r0 = rs * 50


VS = utils.dtft_f(vsrc, s.frequency, 1 / s.dt)
V1 = utils.dtft_f(v1, s.frequency, 1 / s.dt)
I1 = utils.dtft_f(i1, s.frequency, 1 / s.dt)
I1 = I1 * np.exp(-1j * 2 * np.pi * s.frequency * s.dt / 2)

Vp = utils.dtft_f(vp, s.frequency, 1 / s.dt)
Ip = utils.dtft_f(ip, s.frequency, 1 / s.dt)
Ip = Ip * np.exp(-1j * 2 * np.pi * s.frequency * s.dt / 2)

# plt.plot(np.abs(VS))
# plt.plot(np.abs(V1 - VS))
z0 = 50
A1 = (Vp + z0 * Ip) / (2 * np.sqrt(z0))
B1 = (Vp - np.conj(z0) * Ip) / (2 * np.sqrt(z0))
...
# z0 = 50
# A/B Calculation
# I1 = I1 * np.exp(-1j * 2 * np.pi * frequency * s.dt / 2)
# A1 = (V1 + z0 * I1) / (2 * np.sqrt(z0.real))
# B1 = (V1 - np.conj(z0) * I1) / (2 * np.sqrt(z0.real))
# S11 = B1 / A1
...
Zp = Vp / Ip
Sp = conv.gamma_z(Zp)
# 25% of the cell height under the trace seems to be dropped
# 0.02 --> 0.015
...
ax.plot(frequency, conv.db20_lin(msline50(2).evaluate(frequency)["s"].sel(a=1, b=1).squeeze()))
ax.set_ylim([-50, 0])
...
# plt.plot(s.gif_fields_ex[:, 15, p1_y-1, 1])
# plt.plot(s.gif_fields_ex[:, 15, p1_y, 1])
# plt.plot(s.gif_fields_ex[:, 15, p1_y+1, 1])
# plt.plot(s.gif_fields_ex[:, 15, p1_y+2, 1])
