...
class Solver_SingleLayer():
    ...
    def run(self, port, v_waveform, gif_step=10):
        ...
        for n in range(Nt -  1):
            ...
            # ez_x[src_x, src_y, 0] -= (self.dt / self.eps_ez[0]) * (v_waveform[n + 1])
            # ez_y[src_x, src_y, 0] -= (self.dt / self.eps_ez[0]) * (v_waveform[n + 1])
            # ez = ez_x + ez_y
    ...
    def add_port(self, name, ez_x, ez_y, r0=50, ind_comp = 0.75):
        ...
        print(self.Ca["ez_x"][r_x, r_y, r_z])
        print(r_x, r_y, r_z)
...
dz[3:] = conv.m_in(0.02)
dy[p1_y-1: p1_y+1] = conv.m_in(0.005)
dy[p1_y-2] = conv.m_in(0.010)
dy[p1_y+1] = conv.m_in(0.010)
# dy[p1_y-3] = conv.m_in(0.015)
# dy[p1_y+2] = conv.m_in(0.015)
...
s.add_c_probe("probe1", 25, p1_y-1, p1_y)
...
src_v =  0.2e-1 * (np.sin(2*np.pi*f0 * (t)) * np.exp(-((t - t0) / t_half)**2)).astype(np.float32).squeeze()

plt.plot(src_v)

s.run("p1", src_v, gif_step=15)
