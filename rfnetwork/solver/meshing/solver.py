...
class SolverBase():
    ...
    def run(self, ports, v_waveforms):
        ...
        for n in range(Nt - 1):
            ...
            for i, p in enumerate(ports):
                src_x, src_y, src_z, Vs_a, _ = self.ports[p].values()

                ez_x[src_x, src_y, src_z] -= (Vs_a) * (v_waveforms[i][n])
                ez_y[src_x, src_y, src_z] -= (Vs_a) * (v_waveforms[i][n])
    ...
    def add_yPML(self, d_pml=10, side="upper"):
        ...
        raise RuntimeError("Fix MS line exclusions as done in xPML")
    ...
    def add_xPML(self, d_pml=10, side="upper"):
        ...
        sig_pec = 1e7
        ms_z = self.ms_z
        Ca_ms = (2 * self.eps_exy[ms_z] - (sig_pec * dt)) / (2 * self.eps_exy[ms_z] + (sig_pec * dt))
        Cb_ms = (2 * dt) / ((2 * self.eps_exy[ms_z] + (sig_pec * dt)))

        # set ey coefficient to PEC if either cell next to it (along x) is set as copper
        ey_patt = (pattern[:-1] | pattern[1:])[h_idx]

        self.Ca["ey_x"][e_idx, :, self.ms_z] = np.where(
            ey_patt, Ca_ms, self.Ca["ey_x"][e_idx, :, self.ms_z]
        )
        self.Cb["ey_x"][e_idx, :, self.ms_z] = np.where(
            ey_patt, Cb_ms, self.Cb["ey_x"][e_idx, :, self.ms_z]
        )
    ...
    def plot_fields(
        ...
        def callback(t):
            ...
            n_t = (t * 1e-9) / self.dt
            n_f = np.clip(int(n_t // t_step), 0, nframe-1)
            
            for m, f in field_meshes:
                m.point_data["values"][:] = f[n_f]
          
        max_t_ns = nframe * t_step * self.dt * 1e9
    ...
    def get_sparameters(self, z0=50, source_port=1):
        ...
        A = utils.dtft(self.v_src[source_port-1], s.frequency, 1 / s.dt) #* np.exp(1j * 2 * np.pi * s.frequency * s.dt)
...
s.run(["p1", "p2"], [vsrc, -vsrc])

S = s.get_sparameters(source_port=1)

S11 = S[:, 0]
S21 = S[:, 1]

# voltage and current probes at port
vp = s.v_probes["p1_v"]["values"]
i_1 = s.i_probes["p1_i1"]["values"]
i_2 = s.i_probes["p1_i2"]["values"]
ip = (i_1 + i_2) / 2
